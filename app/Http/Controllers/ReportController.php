...
class ReportController extends Controller
{
    ...
    public function index(Request $request){
    	...
    	return view('backend.'.Auth::user()->role.'.report.student', compact('title', 'sexy', 'exam_id', 'section_id'));

    }

    public function cumm(){

        return view('backend.'.Auth::user()->role.'.report.cumm');
    ...
    public function list( Request $request )
    {
        ...
        return view('backend.'.Auth::user()->role.'.report.list', compact('students', 'stu_id', 'exam_id', 'subject', 'subjects', 'section_id', 'class_id', 'running_session', 'school_id', 'marks', 'section', 'total_marks', 'marks_count', 'allresult_data', 'subjectk'))->render();
    } 
    /*
		        ...
		        //dd($students);
		            ...
		            foreach ($students as $SingleStudent) {
		                foreach ($subject as $subjects) {
		                    ...
		                    $marks_string = (empty($marks_string)) ? $FinalMarks : $marks_string . ',' . $FinalMarks;
		                   //dd($marks_string); 
                            $mark_grade = \App\Grade::where([['mark_from', '<=', $FinalMarks], ['mark_upto', '>=', $FinalMarks]])->first();
                            
                            //$gtr = $gh->grade_point * $subjects->gp;
                            //$gt = (empty($gt)) ? $mark_grade->grade_point : $gt . ',' . $mark_grade->grade_point;
		                }
		            ...
		            $total_marks = Mark::where(['student_id' => $stu_id, 'section_id' => $section_id, 'class_id' => $class_id, 'exam_id' => $exam_id, 'session' => $running_session, 'school_id' => $school_id])->sum('mark_total');
		                ...
		                //dd($average_Mark);
						$average_mark = number_format($average_Mark, 2, '.', '');
                        $mark_grade = \App\Grade::where([['mark_from', '<=', $average_mark], ['mark_upto', '>=', $average_mark]])->first();
		                ...
		                $admission_no       =   $SingleStudent->student->code;           //get admission no
                            ...
                            $insert_results->grade              = $mark_grade->grade_point;
